...
export default function libraryLayout({ children }) {
      ...
      icon: (
        ...
        >
          <g stroke="none" strokeWidth={1} fill="none" fillRule="evenodd">
            <g fill="#6b7280" fillRule="nonzero">
              <path
  ...
  return (
    <Layout>
      <div className="min-h-screen bg-primary-6 px-2 py-6 grid grid-cols-5 gap-2">
        <nav className="flex flex-row flex-wrap md:flex-col col-span-5 md:col-span-1">
          {sidebarLinks.map((item, index) => {
            return (
              <Link href={`/u/library/${item.slug}/`} key={index}>
                <a
                ...
                >
                  ...
                  <span className="text-lg mx-1">{item.title}</span>
                </a>
              </Link>
            );
          })}
